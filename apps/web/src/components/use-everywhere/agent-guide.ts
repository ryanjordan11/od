...
export function buildAgentGuideMarkdown(options: AgentGuideOptions = {}): string {
  ...
  lines.push('# OneHub Design — agent setup guide');
  lines.push('');
  lines.push(
    'You are a coding agent (Claude Code, Codex, Cursor, openclaw, hermes, or similar). ' +
      'The user wants OneHub Design wired into your workflow. OneHub Design is a local ' +
  ...
  if (options.versionHint) {
    lines.push('');
    lines.push(`> Reported OneHub Design version: \`${options.versionHint}\``);
  }
  lines.push('');
  ...
  lines.push('   ```');
  lines.push('');
  lines.push('   If it 404s or times out, ask the user to run `pnpm tools-dev` (dev) or open the OneHub Design app (packaged).');
  lines.push('');
